...
const SideBar = ({ places,select,stEnd, direction}) =>{ 
  ...
  for(let i=0;i<places.length;i++){
    ...
    if(writeDirection){
      listItems.push(
        <li className='sidebar-list__item' key={places[i].place_id+"direction"} >
          <p className='sidebar-list__addr'> 距離:{direction.routes[0].legs[j].distance.text} </p>
          <p className='sidebar-list__addr'> 交通時間:{direction.routes[0].legs[j].duration.text} </p>
        </li>
      )
      j++
    }
